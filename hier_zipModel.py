...
for i in range(5):
    ...
    out_traces[i], out_ppc[i], out_smrys[i], err[i] = runModel(df_Train, df_Test, i, 288, 'Connected', 500, 1500)

#%% Output Results
#np.savetxt("data/"+per+"/results/err.csv", err, delimiter=",")

for i in range(5):
    np.savetxt("data/"+per+"/results1/out_ppc"+str(i)+".csv", out_ppc[i], delimiter=",")
    np.savetxt("data/"+per+"/results1/out_smry"+str(i)+".csv", out_smrys[i], delimiter=",")
    #np.savetxt("data/"+per+"/trn_test/out_traces"+str(i)+".csv", out_traces[i])
    
#%%
#np.savetxt("data/"+per+"/results1/names.csv", str(out_smrys[0].columns))
    
text_file = open("sample.txt", "w")
n = text_file.write(str(out_smrys[0].columns))
text_file.close()
